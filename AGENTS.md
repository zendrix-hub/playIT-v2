...
- **Ponytail Anti-Bloat Discipline**: Enforced in `.agents/rules/ponytail.md` and skills (`ponytail`, `ponytail-review`, `ponytail-audit`). The best code is the code never written. YAGNI, standard library first, native platform features before dependencies, shortest clean diff.
- **Real Engineering Skills Active**: Matt Pocock's skills (`implement`, `tdd`, `code-review`, `diagnosing-bugs`, `domain-modeling`, `to-spec`, `to-tickets`, `triage`, `wizard`) are active in `.agents/skills/`.
- **Media & Asset Tooling**: Located in `tools/` with wrappers for `VoiceStudio` (`voice_studio_runner.py`), `whisper` (`whisper_evaluator.py`), and `Fooocus` (`fooocus_asset_bridge.py` + `sdxl_styles_playit.json`).
